...
class TadpoleConvNet:
    ...
    IMG_SIZE = 60       # 28                              # dimension to resize to. should be a multiple of 4.
    IMG_PIXELS = 60*60 # 784  # 28*28
    GRAYSCALE = True
...
def main(_):
    ...
    while os.path.exists("./{}{}/".format(tadconv.model_save, pth_n)):
        ...
        if pth_n >= 100:
            ...
            raise SystemExit("Couldn't find most recent model to test")

    if isinstance(pth_n, int):
        pth_n -= 1  # test the most recent save file name that exists

    path = "{}{}".format(tadconv.model_save, pth_n)

    print("Testing model {}".format(path))

    model_name = "SavedModel{}".format(pth_n)

    tadconv.test_model(path='SavedModels/', model_name=model_name, type_to_restore="SavedModel", show_incorrect=True)
